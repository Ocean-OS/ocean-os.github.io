//JINST App Installer v1.0 Alpha
...
function createWindow() {
    ...
    mainWindow.setTitle("Loading Installer...");
    mainWindow.loadFile("index.html");
    mainWindow.show();
}

app.on('ready', function() {
  fetch("https://ocean-os.github.io/app/jinst/version.txt").then(res => res.text()).then(version =>{
    if(version !== fs.readFileSync(path.join(__dirname, "version.txt"))){
      fetch("https://ocean-os.github.io/app/jinst/dir.js").then(res => res.text().then(files => {
        fs.writeFileSync(path.join(__dirname, "dir.js"), files);
        for(var updateDownload = 0; updateDownload.length < JSON.parse(files.length); updateDownload++){
          fetch("https://ocean-os.github.io/app/jinst/" + files[updateDownload]).then(res=>res.buffer()).then(buff => {
            fs.writeFileSync(path.join(__dirname,files[updateDownload]),buff);
          });
        };
    ...
    mainWindow.webContents.once('dom-ready', () => {
        if (process.argv.length >= 2 && process.platform == "win32") {
            if (fs.existsSync(process.argv[1])) {
                if(path.parse(process.argv[1]).ext == ".jinst"){
                mainWindow.loadFile(loadInstaller(process.argv[1]));    }else{
            mainWindow.loadFile("maker.html");
            }
            } else {
                if(process.argv[1] !== "jinst:///" && process.argv[1] !== "" && process.argv[1] !== "jinst://builder/" && process.argv[1] !== "jinst://builder"){
                ...
                async function loadFromWeb(p) {
                    fetch(decodeURIComponent(p.split("?url=")[1]))
                        .then(res => res.text())
                        .then(text => {
                            if(!fs.existsSync(path.join(app.getPath('temp'), 'jinstFiles'))) fs.mkdirSync(path.join(app.getPath('temp'), 'jinstFiles'));
...
app.on('open-url', (event, url) => {
  fetch("https://ocean-os.github.io/app/jinst/version.txt").then(res => res.text()).then(version =>{
    ...
    if(url == "jinst:///" || url == "jinst://" || url == "jinst://builder/" || url == "jinst://builder"){
        mainWindow.loadFile("maker.html");
    }else{
        loadFromWeb(url);
    }
...
var loadInstaller = function(filepath) {
    ...
    if(typeof fd.appDescription == "undefined") fd.appDescription = ""; else fd.appDescription = `${fd.appDescription}<br>`;
    if(typeof fd.appAuthorLink !== "undefined") if(typeof fd.appAuthorIcon == "undefined"){ fd.appAuthor = `<a href="javascript:shell.openExternal('${fd.appAuthorLink}')">${fd.appAuthor}</a>` }else {fd.appAuthor = `<a href="javascript:shell.openExternal('${fd.appAuthorLink}')"><img src="${fd.appAuthorIcon}" width="15em" height="15em"  style="border-radius:50%">${fd.appAuthor}</a>`}
    ...
    if(generic.includes(s.font)){
        s.font = genericReplace[generic.indexOf(s.font)];
    }
    var b = s.bg_color;
    if (b.split("")[0] == "#") {
        b = hexToRgb(b);
    ...
    var diff = s.button_cc == "lighter" && s.color.toLowerCase() !== "#ffffff" ? 20 : -20;
    var style = `
    ...
    a:hover{
        color: rgb(${hexToRgb(s.color).r+diff},${hexToRgb(s.color).g+diff},${hexToRgb(s.color).b+diff});    
    }`;
    var script = `
    const fs = require('fs');
    const path = require('path');
...
function extractZipFile(zipFilePath, outputDir) {
  ...
  try{
    ...
    if(err.code == "ENOSPC"){
      alert("You don't have enough storage space to download this app.");
    }
  }
}
...
while (true) {
  ...
  progress = 10+(20*percentComplete.toFixed(2));
  setProgress();
}
...
try{
    ...
    var createPage = `<head><style>${style}</style><title>${fd.appName} Installer</title></head><body><h1><img src="${fd.appIcon}" width="28em" height="28em">Install ${fd.appName}</h1><h3>App Details</h3>Author: ${fd.appAuthor}<br>${fd.appDescription}<div id="progress" hidden><progress id="progressbar" max="100" value="0"></progress><br><div id="progressState"></div></div><button onclick="install()" id='install'>Install</button>&nbsp;&nbsp;&nbsp;<button onclick="window.close()">Cancel</button><script>${script}</script></body>`;
...
process.on('uncaughtException', function(error) {
    if(!fs.existsSync(path.join(__dirname, "logs.txt"))) fs.writeFileSync(path.join(__dirname, "logs.txt"), "");
    fs.writeFileSync(path.join(__dirname, "logs.txt"), fs.readFileSync(path.join(__dirname, "logs.txt"), "utf8") + error + "\n");
});